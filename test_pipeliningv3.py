...
for iteration_idx in range(num_iterations):
    ...
    batched_sampled_ids_cpu = [batched_sampled_ids[i].cpu() for i in range(num_micro_batches)]
    ...
    for i in range(num_micro_batches):
        ...
        with torch.cuda.stream(comm_stream):
            ...
            tmp = torch.empty((batched_sampled_ids[i].shape[0], M),
                               dtype=torch.float32,
                               device='cuda')
            tmp = tmp * 2
            # create an event
        ...
        with torch.cuda.stream(comm_stream):
            ...
            tmp = torch.empty((batched_sampled_ids[i].shape[0], M),
                               ...
                               device='cuda')
            tmp = tmp * 2
            # data_cpu_grad_buffer[batched_sampled_ids[i]] = data.grad.cpu()
...
# nsys profile --force-overwrite true -o test_pipeliningv3 python test_pipeliningv3.py
