...
for iteration_idx in range(num_iterations):
    ...
    torch.cuda.nvtx.range_push("iteration_idx: " + str(iteration_idx))
    ...
    for i in range(num_micro_batches):
        data_all.append(torch.empty((batched_sampled_ids[0].shape[0], M),
                                    dtype=torch.float32,
                                    device='cuda',
                                    requires_grad=True))
    ...
    with torch.cuda.stream(comm_stream):
        # Forward pass
        load_param_from_cpu_stream(data_cpu, data_all[0], batched_sampled_ids[0], N, M)
        # create an event
    ...
    for i in range(num_micro_batches):
        ...
        torch.cuda.nvtx.range_push("micro_batch_idx: " + str(i))
        ...
        for _ in range(10):
            ...
            loss += torch.sum(data_sq2)
        loss = loss / 10 / batched_sampled_ids[0].shape[0] / batched_sampled_ids[0].shape[0]
        ...
        with torch.cuda.stream(comm_stream):
            ...
            if i < num_micro_batches - 1:
                load_param_from_cpu_stream(data_cpu, data_all[i+1], batched_sampled_ids[i+1], N, M)
                # create an event
                cpu2gpu_event = torch.cuda.Event(enable_timing=True)
                cpu2gpu_event.record(comm_stream)
